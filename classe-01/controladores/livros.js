...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        const { rowCount } = await conexao.query('select * from emprestimos where livro_id = $1', [id]);

        if (rowCount > 0) {
            return res.status(400).json('Este livro foi emprestado, não pode ser excluído.');
        }
